...
Instrucciones de trabajo para **Codex** (el equivalente para Claude es `CLAUDE.md`; ambos comparten método).
...
- Versión actual en producción: **v9.5.23** (la real la marca `SCRIPT_VERSION` en `index.html`; no fiarse de este número si el código dice otra cosa).

> Aviso de flujo: el repositorio se actualiza a veces por la web de GitHub ("Add files via upload") además de por git local. Antes de commitear, ejecuta `git fetch origin` y comprueba si el clon local está por detrás de `origin/main`; sincroniza si hace falta (`git reset --hard origin/main` es seguro si el `index.html` del working tree ya es idéntico al de origin).
...
- Para revisar una versión concreta, usa `rg` y `git diff --no-index` sobre archivos concretos; no abras toda la carpeta.
...
- `extra` = `xhigh`
- `máxima` = `max`
- `ultracode` = modo de Codex, no un nivel estándar de API; equivale a `xhigh` con permiso para flujos multiagente/subagentes.
...
1. **Consulta, redacción de prompt, comandos Git o explicación breve** — Opus, nivel `baja`.
   Ejemplos: redactar prompt para Codex, preparar comandos PowerShell, explicar un flujo, revisar una instrucción corta.
...
5. **Lógica crítica Trimble** — Opus, nivel `alta` o `extra`.
   Ejemplos: colores Trimble/Tekla/IFC, selección de objetos, API Trimble, etiquetas 3D, Quantity Surveyor, Configurar vista, apertura/cambio de vistas, export/import CSV, exportación masiva, ZIP, cachés globales.
...
La selección de nivel debe basarse en la documentación oficial actual sobre `effort` cuando esté disponible:
...
- `ultracode` en Codex debe tratarse como modo especial, no como nivel de API.

Si la documentación o la UI de Codex cambia, aplicar la regla conservadora:
...
- Solo puede animarse mientras trabaja (giro de flechas; halo azul parpadeante durante la carga inicial de datos del modelo).
- Debe evitar clicks concurrentes.
- Debe registrar errores en debug/recordError, no ensuciar la UI.
- Mantener botones locales solo si son realmente necesarios: Actualizar vistas, Cancelar (mientras haya proceso activo) y el botón de Bug Report.
...
- Detección de colores nativos IFC/Tekla: se intenta extraer el color desde las propiedades del objeto además de la presentación; si la API no lo expone, indicarlo como limitación técnica y registrarlo en debug.
- No afirmar que un color funciona si no se puede verificar.

---

## Apertura/cambio de vistas

- La activación de una vista NO se considera correcta solo porque un método de la API no lance error.
- Debe confirmarse con `verifyActiveView` (por id, nombre o description con tolerancia) tras cada intento.
- `activateViewById` prueba métodos (`setView/selectView/setCurrentView/activateView/applyView/goToView`) y payloads (id string primero), verificando cada uno; expone diagnóstico en `lastViewActivationDiagnostics`.
- Si ningún método confirma, indicarlo en el log; no asumir que la vista se abrió. La exportación masiva depende de este núcleo.
...
- Selección cercanos (incluido el atajo de teclado configurable).
- Selección ocultos / Ver ocultos.
- Mostrar parámetros / Etiquetas 3D.
- Inspector de selección.
- Quantity Surveyor.
- Configurar vista / "Compartir con" con chips/autocompletado / buscador "Vista activa" / Ir a vista.
- Núcleo de apertura/cambio de vistas (`activateViewById`, `verifyActiveView`, `setActiveViewCandidate`).
- Exportación CSV (simple) / Importación CSV (con color configurable, magenta RFI por defecto).
- Exportación masiva (ZIP, modo permisivo/estricto, `_informe.txt`).
- Botón global "Actualizar datos del visor".
- Bug Report (capturas asistidas, pegado de imagen, anotaciones, menciones de UI, descarga ZIP con TXT/JSON/imágenes/manifest + `reported_bugs.json/.txt/_summary.json`).
- Reported bugs (ventana de seguimiento en el panel Desarrollador) y snapshot de debug (`collectDebugSnapshot`).
